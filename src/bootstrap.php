...
//  */
// /**
//  * Minifica saída HTML em produção preservando tags essenciais.
//  */
// function minify_html(string $html): string
// {
//     $placeholders = [];
//     $html = preg_replace_callback('#<(pre|textarea|script)[^>]*>.*?</\1>#is', function ($matches) use (&$placeholders): string {
//         $key = '___HTML_P_' . count($placeholders) . '___';
//         $content = $matches[0];
//         if (str_starts_with(strtolower($matches[1]), 'script')) {
...
//     $html = preg_replace('/>\s*(___HTML_P_\d+___)\s*/', '>$1', $html) ?? $html;
//     $html = preg_replace('/\s*(___HTML_P_\d+___)\s*</', '$1<', $html) ?? $html;
//     $html = preg_replace('/>\s+</', '><', $html) ?? $html;
//     if ($placeholders !== []) {
